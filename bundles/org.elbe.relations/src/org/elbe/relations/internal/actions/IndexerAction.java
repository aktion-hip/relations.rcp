...
public class IndexerAction extends Action {
    ...
    /** Setter for silent switch. If Action is set silent, no message to confirm the start of the job is displayed.
     *
    ...
    public void run() {
        ...
        final RelationsIndexer indexer = RelationsIndexerWithLanguage.createRelationsIndexer(this.context);
    ...
    private void indexWithFeedback() {
        final IEclipseContext context = this.context.createChild();
        final IndexJob job = new IndexJob(context, this.dataService, this.log);
        final ProgressMonitorDialog dialog = new ProgressMonitorDialog(this.shell);
        dialog.open();
        try {
    ...
    private class IndexJob implements IRunnableWithProgress {
        ...
        public IndexJob(final IEclipseContext context, final IDataService dataService, final Logger logger) {
        ...
        public void run(final IProgressMonitor monitor) {
            this.context.set(IProgressMonitor.class.getName(), monitor);
            ...
            try {
                this.indexed = indexer.refreshIndex(progress);
            } catch (final IOException | VException | SQLException exc) {
                this.log.error(exc, exc.getMessage());
            } finally {
                monitor.done();
            }

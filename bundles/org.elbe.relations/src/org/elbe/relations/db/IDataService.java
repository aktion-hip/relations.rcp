...
public interface IDataService {
	 ...
	 */
	void loadNew(final LightWeightTerm term);
	 ...
	 */
	void loadNew(final LightWeightText text);

	/**
	 * Adds the newly created person item to the relevant collections and sends a
	 * notification.
	 *
	 * @param person {@link LightWeightPerson}
	 */
	void loadNew(final LightWeightPerson person);

	/**
	 * Loads the data from the configured data store.
	 *
	 * @param eventTopic String the event topic to post after data loading has
	 *                     been done
	 */
	void loadData(final String eventTopic);
	 ...
	 * @throws BOMException
	 */
	IItemModel retrieveItem(final UniqueID id) throws BOMException;

	/**
	 * Removes the deleted item from the relevant collections and notifies the
	 * listeners.
	 *
	 * @param item ILightWeightItem
	 */
	void removeDeleted(final ILightWeightItem item);

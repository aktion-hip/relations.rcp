...
public final class DropDataHelper {
    // @formatter:off
    public static final Transfer[] DRAG_TYPES = new Transfer[] { ItemTransfer.getInstance(null) };
    public static final Transfer[] DROP_TYPES = new Transfer[] { ItemTransfer.getInstance(null), FileTransfer.getInstance(), URLTransfer.getInstance() };

    public enum TransferDropHandler {
        ITEM_TRANSFER(ItemTransfer.getInstance(null), new ItemTransferDropHandler()),
        FILE_TRANSFER(FileTransfer.getInstance(), new FileTransferDropHandler()),
        URL_TRANSFER(URLTransfer.getInstance(), new URLTransferDropHandler());
        // @formatter:on
    ...
    private static class ItemTransferDropHandler implements IDropHandler {
        ...
        public void handleDrop(final Object eventData,
                final IAssociationsModel model, final IEclipseContext context) {
            if (eventData instanceof final UniqueID[] uniqueIDs) {
                addAsRelations(uniqueIDs, model, context);
            }
        }
    }
